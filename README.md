...
=======
Next I made an attempt at a genre classification model. Using all information available at a songs release, we want to be able to classify what genre it is. I tried a couple different models which all had low degrees of success for classification (0.3 validation acc), so I decided to simplify the problem and design a model to tell if a particular song is classified as Pop. First I prepared the data for training by making sure all features were normalized. Then I tested several different Classification models (Decision Tree, KNN, MLP, Log Reg). Out of all these models, the two best performing models were the K-Nearest-Neighbors and Decision Tree, with KNN accuracy averaging around 84% and Decision Tree averaging around 75% (both on validation data). Then I realized that the Decision Tree model was overfitting a lot (should have realized this quicker), and maybe I could tune some hyperparameters to improve that accuraccy a bit. I found through some trial and error that
So I did, and when I did, Decision Tree validation accuraccy was hitting the low 80s and with a Decision Tree / K-Nearest Neighbors ensemble I was consistently hitting 85-89% accuracy on validation data.
